        ...
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div v-if="tickerLength && optionTickers.length"
              ...
              <span
                  v-for="(ticker, i) in optionTickers"
                  :key="i"
                  @click="add($event, ticker)"
                  class="inline-flex items-center px-2 m-1 rounded-md text-xs font-medium bg-gray-300 text-gray-800 cursor-pointer">
                {{ ticker }}
              </span>
            </div>
            <div v-if="tickerError" class="text-sm text-red-600">Такой тикер уже добавлен</div>
            <div v-if="badTickerError" class="text-sm text-red-600">Такого тикера нет в базе</div>
...
export default {
  ...
  async created() {
    ...
    this.coinList = Object.values(coinList.Data).map(coin => {
      delete coin.Id
      delete coin.ImageUrl
      return coin
    })
    this.loading = false
  },
  computed: {
    ...
    optionTickers() {
      ...
      for (const t of this.coinList) {
        if (t.Symbol === this.ticker.toUpperCase()) {
          options.push(t.Symbol)
        }
      }
      for (const t of this.coinList) {
        if (t.Symbol.includes(this.ticker) || t.FullName.includes(this.ticker)) {
          if (!options.includes(t.Symbol)) {
            options.push(t.Symbol)
          }
  ...
  methods: {
    ...
    async add(_, ticker) {
      const currentTicker = {
        name: ticker || this.ticker.toUpperCase(),
        price: '-'
      }
      ...
      this.badTickerError = !this.coinList.some(t => t.Symbol === currentTicker.name)

      if (!this.tickerError && !this.badTickerError) {
